...
public class IGRPhotoTweakView: UIView {
    ...
    public var photoContentViewFrame: CGRect {
        get {
            let rect: CGRect = self.photoContentView.convert(self.photoContentView.bounds,
                                                             to: self)
            return rect
    ...
    internal var rotateCnt: Int         = 0
    internal var fliped: Bool           = false
    ...
    public func resetView() {
        UIView.animate(withDuration: kAnimationDuration, animations: {() -> Void in
            ...
            self.photoContentView.transform = CGAffineTransform.identity
    ...
    public func setupPriorValues(_ cropSize: CGSize, _ contentViewFrame: CGRect, _ scrollViewFrame: CGRect, _ scrollViewContentOffset: CGPoint, _ rotateCnt: Int) {
        ...
        //self.photoContentView.frame = contentViewFrame
        let scaleX = contentViewFrame.size.width / self.photoContentView.bounds.width
        let scaleY = contentViewFrame.size.height / self.photoContentView.bounds.height
        
        let translationX = (scaleX - 1.0) * self.photoContentView.bounds.width / 2.0
        let translationY = (scaleY - 1.0) * self.photoContentView.bounds.height / 2.0

        self.photoContentView.transform = CGAffineTransform(scaleX: scaleX, y: scaleY).translatedBy(x: translationX / scaleX, y: translationY / scaleX)
        
        self.cropView.frame = CGRect(origin: .zero, size: cropSize)
        self.cropView.center = self.scrollView.center
